...
modif_Equip(){
 ...
 mysql -u UserMygest -p"$PASSWORD" ma_bd -e "$requete"
...
modif_TypeE(){
 ...
 if [ $id_verif ]; then
 # on modifie
 echo "saisir le nouveau identifiant"
     read -p "nouveau identifiant " libelle
 echo "saisir le nouveau nom"
     read -p "nouveau  nom " libelé
 result="update TyPeE set id = '$id',libelle= '$libelle'"
 mysql -u UserMygest -p"$PASSWORD" ma_bd -e "$result"
 echo "TypeE modifier avec succés" 
 else
 echo "Aucun type d'équipement trouvé avec cet id !"
 fi
}
